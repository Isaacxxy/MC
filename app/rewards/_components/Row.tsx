...
const Row = ({
  ...
  const totalPointsRequired = coupons.reduce(
    (sum, coupon) => sum + coupon.pointsRequired,
    0
  );
  ...
  return (
    <div
      className={`relative flex h-80 w-full flex-col rounded-lg p-4 ${
        isWallet ? "border border-dashed border-black" : "bg-[#ecd4c5]/30"
      }`}
    >
      {isWallet && (
        <div className="-z-10">
      ...
      <div
        ref={setNodeRef}
        className={`flex flex-auto flex-row gap-4 ${
          isWalletEmpty && "items-center justify-center"
        }`}
      >
        {isWalletEmpty ? (
          <p className="text-gray-500 text-lg font-medium bg-white dark:bg-black p-6 rounded-full">
            DRAG and DROP a coupon here
          </p>
        ) : (
          coupons.map((coupon) => (
      ...
      {hasCoupons && (
        <div className="mx-auto mt-5">
          <button
            ...
            className={`px-4 py-2 text-white rounded-lg transition-colors shadow-md capitalize ${
              hasEnoughPoints
